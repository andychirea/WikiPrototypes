...
namespace WikiPrototypes
{
    public static class IrregularSidePartBuilder
    {
        ...
        public static Curve[] GetEndHoles(double posX, double posY, double rotation, double offset)
        {
            var rot90 = Math.PI * .5;

            var result = new Curve[2];

            result[0] = ConnectorBuilder.GetRoundCutOff(2.574, 0.45, posX - 12.95, posY + 5.213 + offset, rot90);
            result[1] = ConnectorBuilder.GetRoundCutOff(2.574, 0.45, posX + 12.95, posY + 5.213 - offset, rot90);
